...
const ModalCameraCheia: React.FC<Props> = ({ visivel, aoFechar, onFotoCapturada }) => {
    ...
    const abrirGaleria = async () => {
        try {
            ...
            if (!result.canceled && result.assets.length > 0) {
                const fotoGaleriaUri = result.assets[0].uri;
                console.log('✅ Foto escolhida da galeria:', fotoGaleriaUri);
                onFotoCapturada(fotoGaleriaUri); // envia para a tela principal
                aoFechar(); // fecha o modal
            }
        } catch (error) {
...
export default ModalCameraCheia;
