...
const ModalCategorias: React.FC<Props> = ({ visivel, aoFechar, aoSelecionarCategoria, categoriaAtual }) => {
  ...
  useEffect(() => {
    if (visivel) {
      alturaAnimadaDespesas.setValue(1); 
      alturaAnimadaReceitas.setValue(1);
      setAlturaDespesasVisivel(true);
      setAlturaReceitasVisivel(true);
    }
  }, [visivel]);
  
  useEffect(() => {
    if (categoriaAtual && visivel) {
      setCategoriaSelecionadaId(categoriaAtual.id);
    }
  }, [categoriaAtual, visivel]);
