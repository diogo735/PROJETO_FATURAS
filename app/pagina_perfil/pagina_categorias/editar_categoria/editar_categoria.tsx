...
import ModalCores from '../criar_categoria/componentes/modal_cores';
...
import ModalAguardeAtualizar from './model_sucesso_editar_categoria';

import ModalCategorias from './modal_categorias_editar';
...
const EditarSubCategoria: React.FC = () => {
    const navigation = useNavigation();

    const route = useRoute<RouteProp<RootStackParamList, 'EditarSubcategoria'>>();
    const { id_subcategoria } = route.params;
    ...
    useEffect(() => {
        const carregarDados = async () => {
            setCarregando(true);
            const sub = await buscarSubCategoriaPorId(id_subcategoria);
            if (sub) {
                setNomeCategoria(sub.nome_subcat);
                setIconeSelecionado(sub.icone_nome);
                setCorSelecionada(sub.cor_subcat);
                setCategoriaSelecionada(sub.categoria_id);

                const cat = await buscarCategoriaPorId(sub.categoria_id); // 👈 AQUI
                setCategoriaInfo(cat);

                setDadosOriginais({
                    nome: sub.nome_subcat,
                    cor: sub.cor_subcat,
                    icone: sub.icone_nome,
    ...
    const iconesCategoria = Array.from(new Set([
    ...
    const modificouAlgo =
        nomeCategoria.trim() !== dadosOriginais.nome.trim() ||
        corSelecionada !== dadosOriginais.cor ||
        iconeSelecionado !== dadosOriginais.icone ||
        categoriaSelecionada !== dadosOriginais.categoriaId;

    const podeCriar = nomeCategoria.trim().length > 0 && corSelecionada !== '#D9D9D9' && iconeSelecionado !== null;
    const [modalSairVisivel, setModalSairVisivel] = useState(false);

    const temAlteracoes = () => modificouAlgo;
            ...
            <View style={styles.container}>
                ...
                <View style={styles.footer}>
                    <TouchableOpacity
                        style={[
                            styles.botaoFinal,
                            { opacity: modificouAlgo ? 1 : 0.5 }
                        ]}
                        disabled={!modificouAlgo}
                        onPress={() => {
                            if (!modificouAlgo) return;
                            setModalCriarVisivel(true);
                        }}
                    >
                        <IconCheck width={16} color="#fff" />
                        <Text style={styles.textoBotaoFinal}>Guardar Alteraçoes</Text>
                    </TouchableOpacity>
                ...
                <ModalCategorias
                    ...
                    aoSelecionarCategoria={(categoriaId) => {
                        ...
                        if (categoriaId) {
                            buscarCategoriaPorId(categoriaId).then(setCategoriaInfo);
                        }
                    }}
                />
